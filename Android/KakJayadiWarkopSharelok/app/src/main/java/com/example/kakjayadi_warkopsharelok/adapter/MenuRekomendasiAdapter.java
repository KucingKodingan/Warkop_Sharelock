...
public class MenuRekomendasiAdapter extends RecyclerView.Adapter<MenuRekomendasiAdapter.MenuViewHolder> {

    ArrayList<MenuRekomendasiModel> arrayListMenuRekomendasi;
    AdapterListener listener;
    Context context;
    View rootView;

    public MenuRekomendasiAdapter(ArrayList<MenuRekomendasiModel> arrayListMenuRekomendasi, AdapterListener listener) {
        this.arrayListMenuRekomendasi = arrayListMenuRekomendasi;
        this.listener = listener;
    }

    @NonNull
    @Override
    public MenuRekomendasiAdapter.MenuViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull MenuRekomendasiAdapter.MenuViewHolder holder, int position) {
        holder.item_list_gambar_menu.setImageResource(R.drawable.cappucino_ice);
        holder.item_list_nama.setText(arrayListMenuRekomendasi.get(position).getNama_menu());
        AngkaRibuan obj = new AngkaRibuan();

        obj.setAngkaRibuan(arrayListMenuRekomendasi.get(position).getHarga().length(), arrayListMenuRekomendasi.get(position).getHarga());
        holder.item_list_harga.setText(obj.getAngkaRibuan());

        String gambar = arrayListMenuRekomendasi.get(position).getGambar();
        Picasso.get()
                .load(ApiService.URL+"/warkop_sharelock/image/menu/"+gambar)
                .error(R.drawable.no_image)
                .into(holder.item_list_gambar_menu);

        MenuRekomendasiModel menu = arrayListMenuRekomendasi.get(position);

        if(arrayListMenuRekomendasi.get(position).getKapasitas().equals("ada")){
        ...
        holder.item_list_btn_menu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                listener.onClickRekomendasi(menu);
            }
        });
    }
    ...
    public int getItemCount() {
        return arrayListMenuRekomendasi.size();
    }
    ...
    public interface AdapterListener{
        void onClickRekomendasi(MenuRekomendasiModel menuModel);
    }
}
